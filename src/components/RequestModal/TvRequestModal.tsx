...
const messages = defineMessages('components.RequestModal', {
  adultSeriesTitle: 'Adult series are not supported',
  adultSeriesDescription:
    'Whisparr handles adult movies and scenes only, so Seerr has no server to send adult series to. Adult movies are unaffected.',
...
const TvRequestModal = ({
  ...
  return data && !error && !data.externalIds.tvdbId && searchModal.show ? (
    ...
    <Modal
      ...
      backdrop={`https://image.tmdb.org/t/p/w1920_and_h800_multi_faces/${data?.backdropPath}`}
    >
      {data?.adult && !editRequest ? (
        <Alert
          type="warning"
          title={intl.formatMessage(messages.adultSeriesTitle)}
        >
          {intl.formatMessage(messages.adultSeriesDescription)}
        </Alert>
      ) : null}
      {editRequest
        ? isOwner
