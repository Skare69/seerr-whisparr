...
export class MediaRequest {
  public static async request(
    ...
    // Adult series have no destination: Whisparr (V3) handles adult movies and
    // scenes only, and its Sonarr-shaped V2 is TPDB-keyed with no TMDB mapping.
    // Reject clearly instead of letting the request fail later on Sonarr.
    if (
      requestBody.mediaType === MediaType.TV &&
      'adult' in tmdbMedia &&
      tmdbMedia.adult
      ...
      throw new Error(
        'Adult series are not supported: Whisparr handles adult movies and scenes only, so Seerr has no server to send them to.'
      );
    }
