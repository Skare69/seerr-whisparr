...
- StashDB scene criteria: `text`, `title`, `url`, `code`, `date`, `production_date`, `studios`, `parentStudio`, `tags`, `performers`, `alias`, `fingerprints`, `favorites`, `has_fingerprint_submissions`, plus `page`/`per_page`/`sort`/`direction`. Sort keys: `TITLE`, `DATE`, `DURATION`, `TRENDING`, `POPULARITY`, `CREATED_AT`, `UPDATED_AT`. There is no duration or year criterion; `date` is a value plus a criterion modifier, so a year filter must be expressed as a date range.
- TPDB `/scenes` and `/movies` accept the same query set: `q`, `title`, `date` + `date_operation`, `year`, `duration`, `site`/`site_id`/`site_operation`, `performers`/`performer_id`/`performer_and`, `performer_genders`/`performer_gender_and`/`performer_gender_only`, `tags`/`tag_and`, `directors`/`director_id`/`director_and`, `category_id`, `external_id`, `hash`/`hashType`, `sku`, `url`, `is_collected`, `is_favourite`, `orderBy`, `page`, `per_page`. Confirmed: no tag-exclusion parameter, and TPDB does have the year/duration inputs StashDB lacks.
- TPDB performer filmographies are separate endpoints — `/performers/{id}/scenes`, `/performers/{id}/movies`, `/performers/{id}/jav` — and take only `page`/`per_page`: a filmography cannot be filtered server-side.
...
- **Credential blocker cleared (2026-09-10):** `TPDB_API_TOKEN` and a READ-capable `STASHDB_API_KEY` were supplied and stored in the ignored `.env.local`. Both were pasted in chat, so both should be rotated once testing settles. What M0 still needs is the Whisparr instance URL and API key, selected real test items, and explicit approval for the first controlled add.
...
- **Still unproven on the provider side:** account tier/quota and rate limits — neither API returned `x-ratelimit-*` headers, so throughput must be measured or read from the provider's terms, not assumed; hosted TPDB works-endpoint accuracy at scale; and every delivery case (TPDB-only movie add, TMDB-precedence bypass, StashDB scene add) which needs the Whisparr instance URL and API key plus an authorized add target.
- **Still blocked on the operator:** the installed homelab Jellyfin's base URL, browser-reachable external URL, administrator API key, owner credentials, and permitted library selection for the live M1 proof on that server; the Whisparr instance URL, API key, an authorized add target and selected real fixtures for the M0 delivery cases; and a Linux/container runner — this workstation has no Docker, Podman, or WSL distro, so the compose build and container smoke path are unexecuted locally and run only in CI.
